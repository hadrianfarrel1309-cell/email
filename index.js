...
async function yahooChart(symbol, range = '3mo', interval = '1d') {
  ...
  if (!result) throw new Error(`Data Yahoo chart kosong untuk ${symbol}`);
  return result;
}

async function yfQuote(symbol) {
  const result = await yahooChart(symbol, '5d', '1d');
...
async function yfHistorical(symbol) {
  const result = await yahooChart(symbol, '3mo', '1d');
...
async function yfSearch(query, { newsCount = 5 } = {}) {
  const url = `https://query1.finance.yahoo.com/v1/finance/search?q=${encodeURIComponent(query)}&newsCount=${newsCount}&quotesCount=0&lang=en-US&region=US`;
  const json = await fetchJson(url);
  return { news: json.news || [] };
}
...
async function fetchYahooNewsBatch(newsCount = 5) {
  if (!CONFIG.yahooNewsEnabled) return [];
  const allNews = [];
